...
% ASSIGNMENT 06: FOUR-YEAR TRANSFORM
% Sampling parameters
Fs = 96000;           % 96 kHz
N = 192000;           % number of samples
t = (0:N-1)/Fs;       % time vector

% 1) Define db2mag() if needed
db2mag = @(dB) 10.^(dB/20);

% 2) Generate signals
% Frequencies and dB levels
f1 = -20480;   dB1 =  0;      % -20.48 kHz,  0 dB
f2 = -360;     dB2 = -0.8;    %   -360 Hz,  -0.8 dB
f3 =  996;     dB3 = -2;      %    996 Hz,  -2 dB
f4 = 19840;    dB4 = -8;      % 19.84 kHz,  -8 dB
...
% 3) Take the DFT and plot in dB scale
%    (Optional) divide by N to get normalized amplitude
X = fft(x_noisy)/N; 
...
% 4) Given digital filter H(z)
% Poles and zeros from the given expression:
...
% 6) Frequency response with freqz
